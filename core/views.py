...
from .manager_files import DataManage
from .manage_pending_users import PendingRequest
        ...
        if(request.method=='POST'):
            ...
            if request.POST.get('fileDown'):
                ...
                if(request.POST.get('path') == None):
                    ...
                    ruta = os.path.join(request.POST.get('path'),request.POST.get('dir'))
                ...
                ruta = request.POST.get('DirDelPath')
...
class InicioSeion(generic.View):
    ...
    def post(self, request):
        if (request.method == "POST"):
            ...
            check = authenticate(request, username=self.user, password=self.password)
    ...
    def post(self, request):
        if(User.objects.count() == 0):
            ...
            if password_1 == password_2:
                User.objects.create_superuser(username=username, email=email, password=password_1)
                check=authenticate(request, username=username, password=password_1)
                if check is not None:
                    login(request, check)
                    return redirect('/')
            else:
                context = {'error':'las contraseñas no coinsiden'}  
                return render(request, 'create_super_user.html', context)
            ...
            context = userRequest.save_panding_user(username, password_1, password_2, email)
            return render(request, self.template_name, context)
...
class AdminIndex(generic.View):
    ...
    @method_decorator(user_passes_test(staffCheck, login_url='/'))
    def post(self, request):
        pending_users = PendingRequest
        if(request.POST.get('accept')):
            pending_users.accept_user(request.POST['accept'])
        elif(request.POST.get('refuse')):
            pending_users.refuse_user(request.POST['refuse'])
        context = {'pendingUsers': PendingUsers.objects.all()}
        return render(request, self.template_name, context)